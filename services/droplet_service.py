...
class DropletService:
    def create_droplet(username: str, database_type: str):
        ...
        if database_type not in ["mysql", "postgres", "mongo", "redis"]:
            raise HTTPException(status_code=400, detail=f"Database type {database_type} not found, please choose from MYSQL, POSTGRES, MONGO, REDIS")
        try:
            payload = get_payload_request(username=username,database_type=database_type)
